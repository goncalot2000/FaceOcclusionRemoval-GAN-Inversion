...
class Discriminator(nn.Module):
    ...
    def forward(self, input, step=0, alpha=-1):
        for i in range(step, -1, -1):
            ...
            if i == step:
                out = self.from_rgb[index](input) #[1, 512, 256, 256]

            if i == 0:
                out_std = torch.sqrt(out.var(0, unbiased=False) + 1e-8) #[512, 256, 256]
                mean_std = out_std.mean()
                mean_std = mean_std.expand(out.size(0), 1, 4, 4) #[1, 1, 4, 4]
                #mean_std = mean_std.expand(out.size(0), out.size(1), out.size(2), out.size(3)) #[1, 1, 4, 4]
