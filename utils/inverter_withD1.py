...
from models.style_based_GAN import Discriminator
...
class StyleGANInverter(object):
  ...
  def __init__(self,
    ...
    self.D = nn.DataParallel(Discriminator()).cuda()
    self.logger.info(f'Loading pytorch weights from `models/pretrain/stylegan-256px-new.model`.')
    self.D.module.load_state_dict(torch.load('models/pretrain/stylegan-256px-new.model')['discriminator'], strict=False)
  ...
  def invert(self, image, num_viz=0):
    ...
    for step in pbar:
      ...
      if True:
        ...
        d_result = self.D(x_rec)#.permute(0, 2, 3, 1))
