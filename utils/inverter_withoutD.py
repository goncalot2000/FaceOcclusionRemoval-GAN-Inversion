...
#from models.styleGAN2_model import Discriminator
...
class StyleGANInverter(object):
  ...
  def __init__(self,
    ...
    """
    discriminator = Discriminator(size=256).to(self.run_device)
    print("Loading Discriminator...")
    model_ckpt = torch.load('../models/pretrain/550000.pt')
    discriminator.load_state_dict(model_ckpt["d"])
    """
  ...
  def invert(self, image, num_viz=0):
    ...
    for step in pbar:
      ...
      if True:
        #Discriminator loss
        x_rec = self.G.net.synthesis(z)
        d_result = discriminator(x_rec)
        log_message += f', discriminator: {d_result:.3f}'
